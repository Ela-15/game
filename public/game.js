...
function spawnPtcls(x, y, col, n, type) {
  for (let i = 0; i < n; i++) {
    ...
    ptcls.push({ x, y, vx:Math.cos(a)*sp, vy:Math.sin(a)*sp-1.5,
...
function spawnHearts(x, y, n) {
  for (let i=0;i<n;i++) {
    ptcls.push({ x:x+(Math.random()-0.5)*50, y, vx:(Math.random()-0.5)*2,
...
function spawnStars(x, y, n) {
  for (let i=0;i<n;i++) {
    ...
    ptcls.push({ x, y, vx:Math.cos(a)*sp, vy:Math.sin(a)*sp-2,
...
const KS = {};
const PREV_JUMP = {};
window.addEventListener('keydown', e => { KS[e.code]=true; e.preventDefault(); });
